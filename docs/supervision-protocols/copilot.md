...
3. First cycle: run `[ -f __FM_X_MODE_ENV_SH__ ] && . __FM_X_MODE_ENV_SH__; exec bin/fm-watch-arm.sh` as its own attached asynchronous shell task, named (its `description`) exactly `Arm Firstmate watcher`.
   The completion-notification hook recognizes this exact task name; a differently worded description is not recognized as the watcher cycle and its completion notification is silently ignored.
