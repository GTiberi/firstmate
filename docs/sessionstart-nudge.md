...
After refusing a gate agent, the run wrapper alone creates an absent standard `state/` directory when `fm_primary_scope_shape_matches` accepts the wrapper's own installed root and its Git history descends from the immutable Firstmate anchor, then applies the ordinary primary-scope gate.
It never initializes a state path redirected away from that installed root through `FM_ROOT_OVERRIDE`, `FM_HOME`, or `FM_STATE_OVERRIDE`; the history check also keeps a copied file layout in an unrelated repository inert. Thus a fresh clone delivers its first full digest without a hook mutating an unrelated checkout, while the nudge and every other primary-scoped hook retain the existing-state requirement.
...
It proves that a fresh primary clone with no `state/` receives the full digest and initializes its state directory, while an unrelated repository that carries the same `AGENTS.md` and `bin/` hook layout remains silent and untouched.
